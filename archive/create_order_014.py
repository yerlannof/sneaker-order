#!/usr/bin/env python3
"""
Create sneaker order ЗК-014 with per-size analytics.

Based on create_order_013.py — no transit (per user: всё уже пришло на 07.05.2026,
поступления от ЗК-013 проведены, остатки актуальны).

Usage:
    python sneaker-order/create_order_014.py --dry-run
    python sneaker-order/create_order_014.py
"""
...
EXCLUDE_PATTERNS = [
    ...
    '%Бомбер%', '%Парк%', '%Анорак%', '%Платье%', '%Юбк%',
]

# Filter by article range — sneakers occupy 200000-209999 (and 400000+ for some)
SNEAKER_ARTICLE_MIN = 200000
SNEAKER_ARTICLE_MAX = 209999
...
MEN_WEIGHTS = {"40": 0.069, "41": 0.158, "42": 0.240, "43": 0.231, "44": 0.177, "45": 0.100, "46": 0.025}
UNISEX_WEIGHTS = {}
...
def detect_gender(size_sold, size_stock):
    """Detect gender — sales SHARE wins, stock is fallback for new/sparse models."""
    def to_int(s):
        try: return int(float(s))
        except (ValueError, TypeError): return None

    women_sales = sum(qty for sz, qty in size_sold.items()
                      if to_int(sz) in {36, 37, 38, 39})
    men_sales = sum(qty for sz, qty in size_sold.items()
                    if to_int(sz) in {43, 44, 45, 46})
    total_sales = sum(size_sold.values())

    # Sales-driven: one gender dominates if its share >= 40% AND other side is just stray (<=1 sale)
    if total_sales >= 5:
        women_share = women_sales / total_sales
        men_share = men_sales / total_sales
        if women_share >= 0.4 and men_sales <= 1:
            return "women"
        if men_share >= 0.4 and women_sales <= 1:
            return "men"
        if women_share >= 0.2 and men_share >= 0.2:
            return "unisex"

    # Stock fallback for new/sparse models — require at least 2 core pairs
    stock_w = sum(size_stock.get(s, 0) for s in ["36", "37"])
    stock_m = sum(size_stock.get(s, 0) for s in ["43", "44"])
    has_women = stock_w >= 2
    has_men = stock_m >= 2
...
def size_range(gender):
    if gender == "women":
        return [str(s) for s in range(36, 42)]  # 36-41
    elif gender == "men":
        return [str(s) for s in range(40, 47)]  # 40-46
    return [str(s) for s in range(36, 47)]  # 36-46 unisex


def calc_total_pairs(adj_rate, w1):
    """Determine total pairs. W1 only filters out genuinely weak NEW models —
    if current rate is hit-level (>=3/нед), trust it (resurrected hit)."""
    if adj_rate >= 6:
        return 60  # super-hit: 10 boxes — trust current rate, ignore old W1
    elif adj_rate >= 3:
        return 48  # hit: 8 boxes — trust current rate
    # Below hit threshold — apply W1 filter for genuinely weak new models
    if w1 <= 2:
        return 12  # weak/test model — 2 boxes max
    if adj_rate >= 1.5:
...
def calc_size_qty(gender, size_sold, size_stock, weekly_rate, w1, transit_pairs=0):
    ...
    # Blend allocation weights: 50% global + 50% model's actual size mix (smoothed for OOS bias)
    # This fixes "AF1-style" issue where sparse-sales models get global distribution
    total_model_sold = sum(size_sold.values())
    if total_model_sold >= 5:
        smoothed = {sz: size_sold.get(sz, 0) + 0.5 for sz in sizes}
        smooth_total = sum(smoothed.values())
        model_weights = {sz: smoothed[sz] / smooth_total for sz in sizes}
        blended = {}
        for sz in sizes:
            blended[sz] = 0.5 * weights.get(sz, 0.01) + 0.5 * model_weights[sz]
        b_sum = sum(blended.values())
        weights = {sz: v / b_sum for sz, v in blended.items()}
    ...
    # Cap any single size at 50% of total pairs (avoid huge skew like 10/12 in one size)
    cap = max(2, int(total_pairs * 0.5))
    overflow = 0
    for sz in sizes:
        if result[sz] > cap:
            overflow += result[sz] - cap
            result[sz] = cap
    # Redistribute overflow to other sizes that sold for this model
    if overflow > 0:
        candidates = sorted(
            [sz for sz in sizes if size_sold.get(sz, 0) > 0 and result[sz] < cap],
            key=lambda s: -size_sold.get(s, 0)
        )
        i = 0
        while overflow > 0 and candidates:
            sz = candidates[i % len(candidates)]
            if result[sz] < cap:
                result[sz] += 1
                overflow -= 1
            i += 1
            if i > len(candidates) * cap:  # safety
                break
...
def main():
    ...
    articles_data = con.execute(f"""
        ...
        WHERE sale_datetime >= '{start}' AND price > 0 AND {excl}
          ...
          AND TRY_CAST(article AS INTEGER) BETWEEN {SNEAKER_ARTICLE_MIN} AND {SNEAKER_ARTICLE_MAX}
        GROUP BY article
    ...
    for row in articles_data:
        ...
        # Buy price: prefer "Поставщик In", fallback to any supplier (replicas/other suppliers)
        ...
        if buy_price == 0:
            bp_row = con.execute(f"""
                SELECT LAST(price ORDER BY supply_moment)
                FROM supply_positions
                WHERE product_article = '{art}' AND price > 0 AND supply_moment >= '2025-01-01'
            """).fetchone()
            buy_price = float(bp_row[0]) if bp_row and bp_row[0] else 0
