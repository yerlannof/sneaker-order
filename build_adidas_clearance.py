...
OUT_HTML = PROJECT_ROOT / 'sneaker-order' / 'adidas_clearance.html'
OUT_JSON = PROJECT_ROOT / 'sneaker-order' / 'adidas_clearance_data.json'
PHOTO_CACHE = PROJECT_ROOT / 'sneaker-order' / '.photo_cache_sneakers.json'  # общий кеш фото по артикулу

SNAPSHOT_DATE = '20260623'
PRICES_DATE = '20260623'  # исправленный снапшот: sale_price = изначальная "Цена продажи"
...
def main():
    ...
    rows = con.execute(f"""
        WITH stock AS (
          ...
          WHERE TRY_CAST(article AS INTEGER) BETWEEN 200000 AND 209999 AND total_stock > 0
            AND LOWER(product_name) LIKE '%adidas%'
          GROUP BY article
          HAVING SUM(astana_aruzhan) > 0   -- только модели, лежащие в Аружане
        ),
    ...
    # Adidas-дашборд: приоритет — больше всего пар в Аружане сверху (фокус точки),
    # потом по категории (мёртвые/убыточные выше) и заморозке.
    items.sort(key=lambda x: (-x['stock']['aruzhan'], cat_order.get(x['category'], 99), -x['frozen_cost']))

    print("\n4. Фото из МС (догружаем недостающие)...")
    ...
    if not skip_download:
        # Adidas-дашборд маленький (~32 модели) → реально докачиваем фото.
        # Сбрасываем пустые записи кеша по нашим артикулам, чтобы повторить попытку.
        for a in article_to_name:
            if cache.get(a, None) == '':
                del cache[a]
...
def get_preset_discounts() -> dict:
    ...
    try:
        ...
        if not url or not key:
            return {}
        r = requests.get(f"{url}/rest/v1/orders?id=eq.ADIDAS-CLEAR-001&select=items",
...
if (_restored > 0) {
  try { localStorage.setItem('clothing_discounts', JSON.stringify(discounts)); } catch(e) {}
    ...
    new_html = re.sub(r'<title>[^<]+</title>',
                      '<title>Adidas — уценка (Аружан)</title>', new_html)
    # H1 + badge
    new_html = re.sub(r'<h1>[^<]+</h1>\s*<span class="badge badge-info">[^<]+</span>',
                      f'<h1>Adidas — уценка</h1>\n    <span class="badge badge-info">{total_models} моделей в Аружане</span>',
                      new_html)
    # Header meta
    today_s = date.today().strftime('%d.%m.%Y')
    new_html = re.sub(r'<div class="header-meta">Остатки:[^<]*</div>',
                      f'<div class="header-meta">Остатки: {today_s} | Adidas, что лежит в Аружане | скидка в МС = на все склады</div>',
                      new_html)
    # Stats
    cat_count = {}
    ...
    lite_file = PROJECT_ROOT / 'sneaker-order' / 'adidas_clearance_lite.json'
    photos_file = PROJECT_ROOT / 'sneaker-order' / 'adidas_clearance_photos.json'
...
async function loadData() {
  ...
  const r = await fetch('adidas_clearance_lite.json');
  ...
  setTimeout(async () => {
    try {
      const r2 = await fetch('adidas_clearance_photos.json');
...
function resetAllDiscounts() {
  ...
  toast('Сброшено: ' + n + ' скидок');
    ...
    # Adidas-дашборд: отдельный localStorage-ключ и Supabase orderId,
    # чтобы скидки не смешивались с обувным (clothing_discounts/SNEAKERS-001).
    new_html = new_html.replace('clothing_discounts', 'adidas_clearance_discounts')
    new_html = new_html.replace('SNEAKERS-001', 'ADIDAS-CLEAR-001')
